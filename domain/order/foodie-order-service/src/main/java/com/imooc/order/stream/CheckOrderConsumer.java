...
public class CheckOrderConsumer {
    ...
    public void consumeOrderStatuszMessage(OrderStatusCheckBO bean) {
        log.info("received order check request, orderId={}",bean.getOrderID());

        String orderID = bean.getOrderID();
        // 查询订单，判断时间是否超时（1天），超时则关闭交易
        ...
        if (days >= 1) {
            ...
            orderStatus.setOrderId(bean.getOrderID());
            orderStatus.setOrderStatus(OrderStatusEnum.CLOSE.type);
            orderStatus.setCloseTime(new Date());
            int i = orderStatusMapper.updateByPrimaryKey(orderStatus);
            log.info("closed order, orderId={} count={}", bean.getOrderID(),i);
        }
    }
}
